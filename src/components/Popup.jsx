...
const PopUp = ({
  ...
  const [imageUrl, setImageUrl] = useState(null); // New state for the uploaded picture
  ...
  const onchange = (e) => {
    ...
    if (name === "expiry" || name === "date_joined") {
      const formattedDate = formatDate(value);
      setUser({ ...user, [e.target.name]: formattedDate });
    } else {
  ...
  useEffect(() => {
    if (editPopUp) {
      setUser({
        ...intialState,
        ...editableMember,
        firstName: editableMember.name.split(" ")[0],
        lastName: editableMember.name.split(" ")[1],
      });
    } else {
      setUser(intialState);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(message).length > 0) {
      ...
      let Query = "";
      if (editPopUp) {
        Query += `memberid=${user.memberid}&status=${user.status}&`;
      }
      Query += `address=${user.address}&date_joined=${user.date_joined}&expiry=
      ${user.expiry}&member_type=${user.member_type}&name=
      ${user.firstName + " " + user.lastName}&org=
      ${user.org}&picture=https://d3qjlq0xe4f3aw.cloudfront.net/blankmale.jpg`;

      try {
        const url = editPopUp ? "/updateMember" : "/add-member";
        ...
        if (res.status === 200) {
          ...
          setUser(intialState);
          setImageUrl("");
          return setShowPopUp(!showPopUp),  editPopUp && setEditPopUp(!editPopUp);
        }
      } catch (err) {
        console.log(err);
  ...
  return (
    <>
      <main
        id="content"
        role="main"
        className="border-gray-300 border-4 rounded mx-auto mt-[6rem] overflow-auto max-h-[50vh] shadow-sm shadow-white   w-[100%]   max-w-[40rem]  "
      >
        {isLoading && (
          ...
          <div className="flex justify-between items-center px-4 pt-4">
            <div className="pr-4">
              <h2 className="block text-2xl font-bold text-gray-800">
                {
                  editPopUp ? "Edit Member" : "Add New Member"
                }
              </h2>
            </div>
            <div>
              <ImCross
                ...
                onClick={() => {
                  ...
                  editPopUp && setEditPopUp(!editPopUp);
                }}
              />
            </div>
          </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        id=" firstName"
                        name="firstName"
                        className=" bg-white py-2 sm:py-3 px-4 block w-full outline-none border-2 border-blue-200 rounded-md text-sm  shadow-sm"
                  ...
                  <div className="">
                    ...
                    <div className="relative">
                      <input
                        ...
                        name="date_joined"
                        className={` ${
                          user.date_joined ? "text-black" : "text-[#a9a9a9]"
                        } bg-white py-2 sm:py-3 px-4  block w-full outline-none border-2 border-blue-200 rounded-md text-sm  shadow-sm`}
                        aria-describedby="date_joined-error"
                  ...
                  <div className="flex-1">
                    ...
                    <div className="relative">
                      <input
                        ...
                        name="expiry"
                        className={` ${
                          user.expiry ? "text-black" : "text-[#a9a9a9]"
                        } bg-white py-2 sm:py-3 px-4  block w-full outline-none border-2 border-blue-200 rounded-md text-sm  shadow-sm`}
