...
const PopUp = ({
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(message).length > 0) {
      ...
      Query += `address=${user.address}&date_joined=${user.joined_at}&expiry=
      ${user.expired_at}&member_type=${user.member_type}&name=
  ...
  return (
      <>
        <main
            id="content"
            role="main"
            className="border-gray-300 border-4 rounded mx-auto mt-[6rem] overflow-auto max-h-[80vh] shadow-sm shadow-white   w-[100%]   max-w-[30rem]  "
        >
          {isLoading && (
